                               ...
                               lower_bound=-10, upper_bound=10, external_dataset=NA)
{
  if(is.null(intercept))
  {
    ...
    linearformula_p1=paste(beta_vec[2:4], colnames(temp_dataset)[2:4], sep="*")
    linearformula_p2=paste(beta_vec[5:8], colnames(temp_dataset)[5:8], sep="*")
    linearformula2_p1=paste(linearformula_p1, collapse=" + ")
    linearformula2_p2=paste(linearformula_p2, collapse=" + ")
    lp_p0 <- with(temp_dataset, eval(parse(text = paste(beta_vec[1], "*V1"))))
    lp_p1 <- with(temp_dataset, eval(parse(text = linearformula2_p1)))
    lp_p2 <- with(temp_dataset, eval(parse(text = linearformula2_p2)))
    lp <- lp_p0 + exp(lp_p1) - exp(lp_p2)
