...
Phi_hat <- function(a, Trials){
  ...
  Whole <- rbind(Trials[, c("V1", "V2", "V3", "V4", "V5", "trt", "Y", "In_Trial_rf")], Target)
...
DR_split <- function(a, Trials){
  ...
  Whole_S1 <- rbind(Trials_S1[, c("V1", "V2", "V3", "V4", "V5", "trt", "Y", "In_Trial_rf")], Target_S1)
  Whole_S2 <- rbind(Trials_S2[, c("V1", "V2", "V3", "V4", "V5", "trt", "Y", "In_Trial_rf")], Target_S2)
  ...
  hat_p_fit_rf_S1 <- ranger(In_Trial_rf~V1+V2+V3+V4+V5, data=Whole_S1)
  hat_p_rf_S2 <- predict(hat_p_fit_rf_S1, data=Whole_S2, type="response")$predictions

  #In_Trial_rf_S2 <- get_prob(Whole_S2, "In_Trial_S2")
  hat_p_fit_rf_S2 <- ranger(In_Trial_rf~V1+V2+V3+V4+V5, data=Whole_S2)
  ...
  ga_fit_rf_S1 <- ranger(Y_rf~V1+V2+V3+V4+V5+V22, data=Trials_arm_S1)
  ...
  ga_fit_rf_S2 <- ranger(Y_rf~V1+V2+V3+V4+V5+V22, data=Trials_arm_S2)
  ...
  ea1_fit_rf_S1 <- ranger(trt_rf~V1+V2+V3+V4+V5, data=Trials_S1)
  ...
  ea1_fit_rf_S2 <- ranger(trt_rf~V1+V2+V3+V4+V5, data=Trials_S2)
